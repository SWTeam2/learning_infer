...
def get_df(host, database, user, password, DATA_POINTS_PER_FILE):
    ...
    colnames = ['hour', 'minute', 'second',
                'microsecond', 'horiz accel', 'vert accel']
    df = pd.DataFrame()
...
def load_data(host, database, user, password, load_cnt=0):
    ...
    for i in range(0, no_of_files):
        ...
        timestamp = datetime.datetime.min.time().replace(
            hour=df.iloc[idx, 0], minute=df.iloc[idx, 1], second=df.iloc[idx, 2])
    ...
    if load_cnt > 0:
        pkz_file = os.path.join(
            main_dir, 'static', f'{load_cnt-1}tmp_bearing1_4_test_data.pkz')
    ...
    out_file = os.path.join(
        main_dir, 'static', f'{load_cnt}tmp_bearing1_4_test_data.pkz')
